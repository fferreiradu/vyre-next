...
function startReachCounter() {
  ...
  reachCounterInterval = setInterval(() => {
...
function startAnydeskCursor() {
  ...
  anydeskCursorInterval = setInterval(() => {
...
function startTaskManager() {
  ...
  taskManagerInterval = setInterval(() => {
    ...
    if (newW > 80) {
      cpuEl.style.background = 'linear-gradient(90deg,#dc2626,#f97316)';
    }
    else if (newW > 50) {
      cpuEl.style.background = 'linear-gradient(90deg,#7c3aff,#c084fc)';
    }
    else {
      cpuEl.style.background = 'linear-gradient(90deg,#16a34a,#4ade80)';
    }
...
window.handleSubmit = async function(btn) {
  ...
  if (!emailRegex.test(email)) {
    showCenterToast("Digite um email válido.", true);
    return;
  }
  ...
  const slowTimer = setTimeout(() => {
    if (btn.disabled) {
      btn.textContent = "Aguarde, conectando...";
    }
  }, 8000);
